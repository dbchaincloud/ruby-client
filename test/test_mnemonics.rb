...
class KeyTest < Minitest::Test
  ...
  def test_public_key_and_address_generation
    ...
    key = DbchainClient::PrivateKey.new(key_pair[0])
    assert_equal key.public_key.public_key_hex, key_pair[1]

    pub_key = DbchainClient::PublicKey.new(key_pair[1])
    assert_equal pub_key.public_key_hex, key_pair[1]

    assert_equal DbchainClient::Mnemonics.public_key_to_address(key_pair[1]),
